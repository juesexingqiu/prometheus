...
func TestBlockQuerierDelete(t *testing.T) {
	...
	for _, c := range []blockQuerierTestCase{
		...
		},
		{
			...
			ms:   []*labels.Matcher{labels.MustNewMatcher(labels.MatchEqual, "a", "a")},
			exp: newMockSeriesSet([]storage.Series{
				storage.NewListSeries(labels.Labels{{Name: "a", Value: "a"}}, nil),
			}),
			expChks: newMockChunkSeriesSet([]storage.ChunkSeries{
				storage.NewListChunkSeriesFromSamples(labels.Labels{{Name: "a", Value: "a"}}, nil),
			}),
		},
		{
			mint: 1,
...
func TestPopulateWithTombSeriesIterator(t *testing.T) {
	cases := []iteratorCase{
		{
			a:    []tsdbutil.Sample{},
			b:    []tsdbutil.Sample{},
	...
	for i, tc := range cases {
		t.Run(fmt.Sprintf("%v", i), func(t *testing.T) {
			...
			tc.test(t, newPopulateWithTombBaseSeries(f, chkMetas, nil, tc.mint, tc.maxt).toSeriesIterator(false))
		})
	}
}

// Regression for: https://github.com/prometheus/tsdb/pull/97
func TestPopulateWithTombBaseSeriesIterator_DoubleSeek(t *testing.T) {
	...
	it := newPopulateWithTombBaseSeries(f, chkMetas, nil, 2, 8).toSeriesIterator(false)
...
func TestPopulateWithTombBaseSeriesIterator_SeekInCurrentChunk(t *testing.T) {
	f, chkMetas := createFakeReaderAndNotPopulatedChunks(
		[]tsdbutil.Sample{},
	...
	it := newPopulateWithTombBaseSeries(f, chkMetas, nil, 1, 7).toSeriesIterator(false)
...
func TestPopulateWithTombBaseSeriesIterator_SeekWithMinTime(t *testing.T) {
	f, chkMetas := createFakeReaderAndNotPopulatedChunks(
		[]tsdbutil.Sample{sample{1, 6}, sample{5, 6}, sample{7, 8}},
	)

	it := newPopulateWithTombBaseSeries(f, chkMetas, nil, 2, 5).toSeriesIterator(false)
...
func TestPopulateWithTombBaseSeriesIterator_NextWithMinTime(t *testing.T) {
	f, chkMetas := createFakeReaderAndNotPopulatedChunks(
		[]tsdbutil.Sample{sample{1, 6}, sample{5, 6}, sample{7, 8}},
	)

	it := newPopulateWithTombBaseSeries(f, chkMetas, nil, 2, 4).toSeriesIterator(false)
